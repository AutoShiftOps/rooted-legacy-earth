/**
 * gedcomExporter.js — serializes a Rooted family tree back into GEDCOM 5.5.1
 * text format, so users can export their data for use in other genealogy
 * tools (Ancestry, MyHeritage, FamilySearch, Gramps, etc.) — a data
 * portability guarantee that matters for an emotionally sensitive product
 * like this one (see docs/CONSENT_MODEL.md and ROADMAP.md Phase 4).
 *
 * Input shape matches the tree response from GET /api/tree/:rootPersonId —
 * { nodes: [...Person], relationships: [...{type, source, target}] }.
 */
